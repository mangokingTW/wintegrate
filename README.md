...
- **Recordings that show the pointer**: `ContinuousRecorder` draws the cursor into every frame and marks each click with an expanding, fading ring plus a crosshair at the coordinate the click was sent to. A BitBlt of the desktop contains no cursor, so a recording without this shows windows changing and nothing about the input that changed them. It is drawn after the screen grab rather than by an on-screen overlay, which is what makes it work over an activated topmost window: measured with one variable, a third-party visualiser's ring went from 2032 pixels to 0 when the window under it became topmost. Clicks come from a low-level hook, so a click from any source shows up — not only the ones wintegrate injected.
