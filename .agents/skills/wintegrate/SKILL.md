...
# `wintegrate` Automation & Verification Skill

`wintegrate` (**Win**dows + **integrate**) is a CI-first Python automation framework engineered to reliably test Windows desktop applications (Win32, WinForms, WPF, Qt, WinUI 3 / Windows App SDK) on unattended runners (e.g. GitHub Actions `windows-latest`) without flakiness, window focus stealing, or localized string dependencies.
...
1. **Never Invent Custom Polling Loops or Sleep Delays**:
   * ❌ Do not write arbitrary `time.sleep(0.5)` for synchronization or manual while-loops.
   * ✅ Use `Window.find(...)`, `Window.wait_for_new(...)`, `locator.wait_for(state="visible", timeout=...)`, or the `settled(...)` polling pattern. If a timeout occurs, `wintegrate` automatically captures the desktop window census delta, logs it to the journal, and reports the failure signature.
...
2. **Never Hand-Craft CI Summary or Reporting Scripts**:
   * ❌ Do not write custom PowerShell or Python scripts to parse `junit.xml`, scrape logs, or append markdown tables to `$GITHUB_STEP_SUMMARY`. (Custom PowerShell string interpolation also risks `ParserError` on escape characters such as `` `u ``).
   * ✅ Wintegrate has **native built-in reporting** for GitHub Actions (`$GITHUB_STEP_SUMMARY`) and pytest-html:
     - `Session._write_step_summary()`: Writes high-priority failure callouts (`> [!CAUTION]`), step durations, and collapsible `<details>` for passing sessions.
...
3. **Session Lifecycle & Exception Propagation Rule (The Fixture Gotcha)**:
   * ❌ **Anti-Pattern**: Wrapping `Session` inside a standard pytest generator fixture (`@pytest.fixture def session(): with Session(...) as s: yield s`).
     - *Why it breaks*: Pytest catches test body exceptions outside the fixture and resumes the fixture with `next()` during teardown without passing the exception to `Session.__exit__` (`exc_type=None`). `Session` assumes the test succeeded, suppresses failure callouts, records `failed: False` in `RECENT_SESSIONS`, and `pytest_runtest_makereport` (which runs during `when == "call"`) misses the session entirely.
...
4. **WinUI 3 Windows Require Foreground & Content Island Focus**:
   * WinUI 3 / Windows App SDK hosts its XAML tree inside a child island (`DesktopChildSiteBridge`).
   * On unattended CI runners, WinUI 3 windows can deactivate and hide if background processes lack foreground permission.
   * ✅ Call `win.set_foreground(timeout=5.0)` followed by `win.focus_content_island(timeout=5.0)` before asserting or sending keyboard inputs.

5. **Never Rely on Localized Strings**:
   * ❌ `app.find_button(name="確定")` or `win.get_by_role("button", name="OK")`
   * ✅ `win.get_by_automation_id("PrimaryButton")` or `win.get_by_role("edit").first` or `win.get_by_class("Button")`
...
#### Pattern B: Built-in `AppSpec` & `session.app()`
...
### 3. Playwright-Style Locators & Auto-Wait
...
submit_btn = win.get_by_role("button", name="Submit")
...
assert focused.automation_id == "CloseButton", f"Expected CloseButton, got {focused.automation_id!r}"
```

---

### 6. Process Sanitation & Artifact Verification
...
### 7. Keyboard & Mouse Input Chords

```python
from wintegrate.interop import send_hotkey, send_keys, send_char_input
...
send_char_input("Special Unicode: 🚀")
```
...
Install `wintegrate[all]>=0.6.3`, `pytest-html`, and `pytest-github-actions-annotate-failures`:

```yaml
- uses: mangokingTW/wintegrate/.github/actions/setup-windows-gui-test@main
  with:
    wintegrate-version: ">=0.6.3"

- name: Install Wintegrate reporting suite
  shell: pwsh
  run: |
    pip install "wintegrate[all]>=0.6.3" pytest-html pytest-github-actions-annotate-failures
...
### GitHub Step Summary & CI Reporting Features (Native)
* **High-priority failure callouts (`> [!CAUTION]`)**: Failed sessions automatically display the error signature, failing step name, and traceback.
* **Collapsible passed sessions (`<details>`)**: Passing test sessions collapse neatly with step counts and execution durations.
